...
if modo == "📚 Lector de documentos":
    ...
    if id_ssr:
        ...
        busq_doc = st.text_input("📁 Buscar tipo documento:")
        opciones_doc = [c['name'] for c in nivel_1 if busq_doc.lower() in c['name'].lower()]
        tipo_doc = st.selectbox("Selecciona tipo documento:", opciones_doc)
        id_doc = next(c['id'] for c in nivel_1 if c['name'] == tipo_doc)

        nivel_2, archivos_2 = listar_subcarpetas_y_archivos(id_doc)
        if nivel_2:
            busq_sub1 = st.text_input("📁 Buscar subnivel 1:")
            opciones_sub1 = [c['name'] for c in nivel_2 if busq_sub1.lower() in c['name'].lower()]
            subnivel_1 = st.selectbox("Selecciona subnivel 1:", opciones_sub1)
            id_sub1 = next(c['id'] for c in nivel_2 if c['name'] == subnivel_1)

            nivel_3, archivos_3 = listar_subcarpetas_y_archivos(id_sub1)
            if nivel_3:
                busq_sub2 = st.text_input("📁 Buscar subnivel 2:")
                opciones_sub2 = [c['name'] for c in nivel_3 if busq_sub2.lower() in c['name'].lower()]
                subnivel_2 = st.selectbox("Selecciona subnivel 2:", opciones_sub2)
