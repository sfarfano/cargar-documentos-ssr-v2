...
def buscar_carpeta_por_codigo(codigo, id_padre):
    ...
    st.write(f"Buscando carpeta con código: '{codigo}' (normalizado: '{codigo_normalizado}') dentro de ID: {id_padre}")
    ...
    for archivo in resultados:
        if archivo["mimeType"] == "application/vnd.google-apps.folder":
            ...
            if codigo_normalizado in nombre_drive:
                return archivo["id"]
    return None
...
if modo == "📤 Cargar documentos":
    ...
    codigo_carpeta = df[df["Nombre combinado"] == tipo_ssr]["Carpeta SSR"].values[0]
    id_base = buscar_carpeta_por_codigo(codigo_carpeta, DRIVE_FOLDER_ID)

    if not id_base:
        st.error(f"No se encontró la carpeta en Drive para el sistema SSR '{tipo_ssr}'. Verifica que exista dentro de la carpeta base.")
    ...
    if st.button("Subir archivo(s)"):
        if not archivos:
            ...
            for archivo in archivos:
                ...
                nuevo_nombre = f"{codigo_carpeta}_Nivel{nivel-1}_{fecha}_{nombre_archivo}"
